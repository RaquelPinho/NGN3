## Shiny app for sanger sequence quatification
...
getMethod("makeBaseCalls","sangerseq")
Method Definition:

function (obj, ratio = 0.33) 
function (obj, ratio = 0.33) 
{
    ...
    stops <- c(primarypeaks[1:(length(primarypeaks) - 1)] + 0.5 * 
        diffs[2:length(diffs)], primarypeaks[length(diffs)] + 
        0.5 * diffs[length(diffs)])
    primary <- NULL
    secondary <- NULL
    ...
    for (i in 1:length(starts)) {
        ...
        if (is.na(Apeak[2]) & is.na(Cpeak[2]) & is.na(Gpeak[2]) & 
            is.na(Tpeak[2])) 
            next
        ...
        if (length(Bases[!is.na(Bases)]) == 4 | length(Bases[!is.na(Bases)]) == 
            0) {
            primary <- c(primary, "N")
            ...
            Bases2 <- Bases[2:4]
            secondary <- c(secondary, mergeIUPACLetters(paste(sort(Bases2[!is.na(Bases2)]), 
                collapse = "")))
        }
        else {
            primary <- c(primary, Bases[1])
    ...
    obj@peakPosMatrix <- tempPosMatrix[rowSums(!is.na(tempPosMatrix)) > 
        0, ]
    obj@peakAmpMatrix <- tempAmpMatrix[rowSums(!is.na(tempPosMatrix)) > 
        0, ]
...
getAnywhere(getpeaks)
A single object matching ‘getpeaks’ was found
It was found in the following places
  namespace:sangerseqR
with value

function (trace) 
{
    r <- rle(trace)
    indexes <- which(rep(diff(sign(diff(c(-Inf, r$values, -Inf)))) == 
        -2, times = r$lengths))
