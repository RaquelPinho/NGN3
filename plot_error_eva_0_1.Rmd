...
## Making CRISPR_set of the target region will all samples 

This set will include as samples (BL,BLM,PTM) and will focus in a 43 bp region including the guide cut site.

```{r crispr_target, echo=FALSE, include=FALSE}
...
crispr_set_target <- readsToTarget(reads = file.path(metadata$Files), names = metadata$Samples, reference = references[1], target =tgt, target.loc = target_loc[[1]]$target_loc, orientation=orientation, group = metadata$Group)
crispr_set_target
# we can extract the variants counts as matrix.
...
SNV_end <- (max(grep("SNV", row.names(vc_all_target))) + 1)
vc_noSNV <- vc_all_target[SNV_end:(SNV_end+20),]
vc_noSNV <- rbind(vc_all_target[1,],vc_noSNV)
```
and then we can get the summary of occurring mutations. 

```{r summary_mutation_target,echo=FALSE, include=FALSE}
crispr_set_target
```

## Making CRISPR_set for up/downstream regions with all samples 

This set will include as samples (BL,BLM,PTM) and will focus in 43 bp region 300bp up or downstream of the guide cut site.

```{r crispr_up_down,echo=FALSE, include=FALSE}
# upstream region
# choose the target and orientation based on the index of gd and strand of the target region 
tgt_up <- gd_up[1] # for NGN3
orientation <-ifelse(strand(tgt_up) == "-", "opposite", "target")
# we can extract the variants counts as matrix.
crispr_set_up <- readsToTarget(reads = file.path(metadata$Files), names = metadata$Samples, reference = control_up[1], target =tgt_up, target.loc = target_loc[[1]]$target_loc, orientation=orientation, group = metadata$Group)
crispr_set_up
# we can extract the variants counts as matrix.
...
# downstream region
# choose the target and orientation based on the index of gd and strand of the target region 
tgt_down <- gd_down[1] # for NGN3
orientation <-ifelse(strand(tgt_down) == "-", "opposite", "target")
# we can extract the variants counts as matrix.
crispr_set_down <- readsToTarget(reads = file.path(metadata$Files), names = metadata$Samples, reference = control_down[1], target =tgt_down, target.loc = target_loc[[1]]$target_loc, orientation=orientation, group = metadata$Group)
crispr_set_down
# we can extract the variants counts as matrix.
...
and then we can get the summary of occurring mutations.
## upstream
```{r summary_mutation_up}
crispr_set_up
```

## downstream
```{r summary_mutation_down}
crispr_set_down
save(crispr_set_target,crispr_set_up,crispr_set_down, file = "D:/Raquel/Desktop/Post_doc/NGN3/NGN3_R/crispr_sets_up_down.RData" )
```
