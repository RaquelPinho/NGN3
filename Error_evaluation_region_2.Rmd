...
## where vc_all_list is a list of readMatrix for the target. up and downstream regions
## where target_loc is the cut locus for the target and off-target loci
```

## Creating variant counts for the target, upstream and downstream region

```{r vc}
# for the target region
vc_all_target <- vc_all_list_up_down[["target"]]
# for the uspstream region
vc_all_up <- vc_all_list_up_down[["upstream"]]
# for the downstream region
vc_all_down <- vc_all_list_up_down[["downstream"]]
...
### Average Number of alleles with no filtering considering per region

I will exclude the samples with 50% less reads in target region than in the average of the up and downstream region since they have probably a high frequence allele with a big deletion in the target site.

```{r allele_number_per_region }
# Getting reads 
samples_longdel <- which(df$total_reads_target < ((df$total_reads_up + df$total_reads_down)/2)*0.5)
Region <- rep(c("target","upstream", "downstream"), each = nrow(df))
    dataPlot <- data.frame(Region =Region,Number_alleles = c(df$alleles_target,df$alleles_up,df$alleles_down), Samples = rep(rownames(df), 3))
    dataPlot<- transform(dataPlot,Region = factor(Region, levels = c("upstream","target","downstream")))
## 
# Getting the mean
mean <- apply(df[-samples_longdel,c(1,3,5)], 2, mean)
# Now displyaing it in shiny
ui <- fluidPage(
  fluidRow(
    column(12, offset= 0, 
       h4("Samples with possible long deletions"),
       textOutput("longdels"))
  ),
  fluidRow(
    column(12, offset= 0, 
      h4("Allele number per sample per region"),
      plotOutput("barplot"))
  )
)
# define server logic options
server <- function(input, output) {
   output$longdels <- renderText({
   print(paste("The samples with probable long deletions are:",                     rownames(df)[samples_longdel[1]],rownames(df)[samples_longdel[2]],rownames(df)[samples_longdel[3]]))
    })
 output$barplot <- renderPlot({
    p <- ggplot(dataPlot, aes( x= Samples,y = Number_alleles, fill = Region)) + 
     scale_fill_manual(values = wes_palette("GrandBudapest1")[1:3]) +
     geom_bar(stat = "identity") + geom_hline(yintercept = mean, color =  rep(wes_palette("GrandBudapest1")[c(2,1,3)],3)) + 
     facet_wrap(~Region) + theme_pubr() + theme(axis.text.x = element_text(angle = 90,size = 6))
    p
...
## Distribution of number of alleles per region before filtering


```{r allelecounts_distribution }
# Now displyaing it in shiny
ui <- fluidPage(
  fluidRow(
    column(12, offset= 0, 
      h4("Number of Allele distribution"),
      plotOutput("hist"))
  )
)
# define server logic options
server <- function(input, output) {
 ...
 output$hist <- renderPlot({
    p <- ggplot(dataPlot, aes(Number_alleles, fill = Region)) + 
     scale_fill_manual(values = wes_palette("GrandBudapest1")[1:3]) +
     geom_density( alpha = 0.6) +  
     theme_pubr() + theme(axis.text.x = element_text(angle = 90,size = 12))
    p
    },height = 700, width = 1200 )
...
## Filtering proposal of using up and downstream region as reference for back ground control

Since each sample (blastocyst) can only have up to two alleles on each region with minimal frequency of 30% (accounting for PCR bias), I will use as cut-off the frequency that removes the background from the up/downstream region on the same sample. I will not use thisapproach for the samples with possible long deletions BL02,BL04 and BL15.

```{r filtering_up_down }
# Now displyaing it in shiny
## Getting the alleles list after background is removed
list_alleles <- lapply(metadata$Samples, FUN = function(s) filt_up_down(vc_all_list, Sample = s))
names(list_alleles) <-metadata$Samples
df_filter <- lapply(seq_along(list_alleles), function(i) {
  dt_filter <- data.frame(Samples = names(list_alleles)[i], target = length(list_alleles[[i]]$target), upstream = length(list_alleles[[i]]$upstream),downstream = length(list_alleles[[i]]$downstream))
})
...
# define server logic options
server <- function(input, output) {
    dataPlot <- melt(df_filter, id.vars = "Samples", variable_name = "Region")
    names(dataPlot)[3] <- "Number_alleles"
    dataPlot<- transform(dataPlot,Region = factor(Region, levels = c("upstream","target","downstream")))
 output$hist <- renderPlot({
    p <- ggplot(dataPlot, aes(Number_alleles, fill = Region)) + 
     scale_fill_manual(values = wes_palette("GrandBudapest1")[1:3]) +
...
### Average Number of alleles with filtered considering per region
...
# Getting the mean
mean <- apply(df[-samples_longdel,c(1,3,5)], 2, mean)
mean2 <- apply(df_filter[-samples_longdel,2:4],2,mean)
# Now displyaing it in shiny
ui <- fluidPage(
  fluidRow(
    column(12, offset= 0, 
...
server <- function(input, output) {
    ...
    dataPlot2 <- melt(df_filter, id.vars = "Samples", variable_name = "Region")
    names(dataPlot2)[3] <- "Number_alleles"
    #format factor
    dataPlot2<- transform(dataPlot2,Region = factor(Region, levels = c("upstream","target","downstream")))
     output$longdels <- renderText({
   print(paste("The samples with probable long deletions are:",                     rownames(df)[samples_longdel[1]],rownames(df)[samples_longdel[2]],rownames(df)[samples_longdel[3]]))
    })
 output$barplot <- renderPlot({
   p1 <- ggplot(dataPlot1, aes( x= Samples,y = Number_alleles, fill = Region)) + geom_bar(stat = "identity") +  scale_fill_manual(values = wes_palette("GrandBudapest1")[1:3]) + geom_hline(yintercept = mean, color =  rep(wes_palette("GrandBudapest1")[c(2,1,3)],3)) + facet_wrap(~ Region) +  theme_pubr() + theme(axis.text.x = element_text(angle = 90,size = 6))
    p2 <- ggplot(dataPlot2, aes( x= Samples,y = Number_alleles, fill = Region)) + geom_bar(stat = "identity") +  scale_fill_manual(values = wes_palette("GrandBudapest1")[1:3]) + geom_hline(yintercept = mean2, color =  rep(wes_palette("GrandBudapest1")[c(2,1,3)],3)) + facet_wrap(~ Region) +  theme_pubr() + theme(axis.text.x = element_text(angle = 90,size = 6))
